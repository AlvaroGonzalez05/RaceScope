...
\begin{itemize}
    ...
    \item \textbf{Transformer v3} ($d_\text{model} = 384$, 8 capas, 8 cabezas, ventana de 40 vueltas, cuatro cabezas de salida, $\approx \num{14.3e6}$ parámetros). Sustituye el término aditivo por la compuerta multiplicativa descrita en \ref{ssec:arquitectura_transformer}, que reescala la representación mediante un vector sigmoidal aprendido del par (circuito, compuesto) y permite amplificar o atenuar dimensiones de forma independiente. Amplía el contexto al rango habitual de un stint de carrera, incorpora la característica \texttt{stint\_number\_norm} para distinguir el comportamiento entre stints sucesivos y añade la cabeza de coste remanente, que el Motor~1 reutiliza en el filtro de rentabilidad por parada. El \textit{dropout} se fija en cero de forma deliberada: la generalización se obtiene por separación de modelos entre pilotos, no por regularización dentro de cada uno.
\end{itemize}

En paralelo a la transición de v2 a v3, el \textit{rollout} autoregresivo del Motor~2 pasó de procesar las simulaciones de Monte Carlo de manera secuencial a tratarlas como un \textit{batch} compartido en el encoder. La implementación de \texttt{TransformerEncoder} en PyTorch admite una dimensión de \textit{batch} arbitraria sin necesidad de modificar la arquitectura. La latencia por petición de \texttt{/api/strategy} pasó del orden de minutos al de segundos, lo que permitió asumir el coste de la arquitectura más grande sin penalizar el tiempo de respuesta del API.
...
El nivel~4 admite, de forma opcional, una sobreescritura del ritmo base mediante un parámetro \texttt{circuit\_baseline} calculado en tiempo de consulta como la mediana de las vueltas de carrera del circuito para el compuesto solicitado (ecuación~\ref{eq:envelope_lap}). Cuando el llamante lo proporciona, sustituye al literal $\beta_0 = 90~\si{\second}$ por un valor coherente con la dinámica real del trazado. Este mecanismo se activa únicamente cuando el dataset no contiene muestra suficiente para entrenar perfiles a los niveles 1 a~3; en los circuitos presentes en el conjunto actual no se utiliza, pero queda disponible ante la incorporación futura de venues nuevos o pilotos con historial corto.
...
\subsection{Cota de validez física: envoltura del circuito y clamps de seguridad}
\label{ssec:envoltura_circuito}

El modelo paramétrico de \S\ref{sec:modelo_parametrico} y el Transformer de \S\ref{sec:modelo_transformer} se entrenan a partir de datos de telemetría que mezclan vueltas de carrera limpias con vueltas in/out, neutralizaciones residuales y sesiones de práctica. Cuando el ajuste por mínimos cuadrados se aplica a una combinación piloto-circuito-compuesto con muestra reducida o con presencia de ruido estructural, los coeficientes resultantes pueden caer fuera del rango físicamente admisible. Las consecuencias se propagan, sin contención adicional, al motor de estrategia.

\subsubsection{Diagnóstico inicial}
\label{sssec:diagnostico_spread}

Para cuantificar el efecto sobre el sistema en producción, se construyó el script \texttt{scripts/diagnose\_pace\_spread.py}, que para una temporada y circuito dados recorre la totalidad de pilotos en parrilla, ejecuta el pipeline completo de \texttt{generate\_strategies} y registra el \texttt{expected\_time} de la estrategia ganadora junto con los parámetros del perfil utilizado. Sobre el Gran Premio de Baréin de 2023 (Sakhir, $L = 57$ vueltas), las observaciones agregadas mostraron:

\begin{itemize}
    \item Rango de duraciones totales entre pilotos: $[57\text{:}42, 3\text{:}16\text{:}45]$, equivalente a una dispersión de \SI{139}{\minute} (\SI{8343}{\second}) en una carrera donde el spread físicamente esperable, en presencia de pilotos doblados, no supera las decenas de segundos.
...
La solución adoptada deriva una cota de validez directamente de la distribución empírica de tiempos de vuelta de carrera del circuito, independiente de los perfiles paramétricos. La función \texttt{\_circuit\_lap\_envelope} extrae, para cada compuesto $c \in \{\text{SOFT}, \text{MEDIUM}, \text{HARD}\}$, los percentiles
...
\begin{enumerate}
    \item \textbf{Corrección térmica acotada} (\texttt{\_apply\_temperature\_correction}). Cada uno de los términos $\beta_2 \cdot \Delta T_{\text{pista}}$ y $\beta_3 \cdot \Delta T_{\text{aire}}$ se acota individualmente al intervalo $[-\Delta_T, +\Delta_T]$ con $\Delta_T = 3~\si{\second}$ (\texttt{TEMP\_CORR\_CLAMP\_S}). El cap se sitúa por encima del rango fisiológico del compuesto pero por debajo del orden de magnitud de los coeficientes sobreajustados detectados en el diagnóstico. El helper se invoca de manera compartida desde \texttt{\_pace\_table} y desde \texttt{\_precompute\_pace\_curves}, lo que evita que ambos caminos del motor diverjan en su tratamiento de la temperatura.
    ...
    \end{equation}
    con $\alpha_\text{lo} = 0.99$ y $\alpha_\text{hi} = 1.02$ (constantes \texttt{PACE\_BASE\_LO\_FACTOR}, \texttt{PACE\_BASE\_HI\_FACTOR}). El extremo inferior impide que un perfil produzca un ritmo más rápido que el quinto percentil histórico del circuito; el extremo superior limita los excesos del intercept a un 2~\% sobre la mediana de carrera, margen suficiente para acomodar a los pilotos legítimamente lentos del campo.

    \item \textbf{Acotación de la curva de ritmo por vuelta.} En \texttt{\_precompute\_pace\_curves} se aplica un clamp por vuelta al intervalo
    \begin{equation}
        \bigl[t^{(c)}_\text{fast} \cdot \gamma_\text{lo},\; t^{(c)}_\text{slow} \cdot \gamma_\text{hi}\bigr]
        \label{eq:lap_clamp}
    \end{equation}
    con $\gamma_\text{lo} = 0.98$ y $\gamma_\text{hi} = 1.10$ (\texttt{PACE\_LAP\_LO\_FACTOR}, \texttt{PACE\_LAP\_HI\_FACTOR}), tanto sobre la \textit{base series} que sirve de contexto al Transformer como sobre los tiempos que devuelve \texttt{model.predict\_stint}. El mismo clamp se aplica dentro del Motor 2 a la matriz \texttt{lap\_times\_batch} producida por los \textit{rollouts} batched de las variantes v1, v2 y v3 del Transformer, antes de su agregación. Este paso protege la suma $\sum_k t_k$ que alimenta a \texttt{\_analytical\_eval} y a \texttt{\_simulate\_strategy} frente a salidas anómalas del modelo en contextos poco representados durante el entrenamiento.

    \item \textbf{Envoltura del tiempo total.} Antes de añadir cada estrategia al payload final, el Motor 3 contrasta el \texttt{expected\_time} contra la cota
    \begin{equation}
    ...
    \end{equation}
    siendo $A = L \cdot \mathrm{median}_c\{t^{(c)}_\text{median}\}$ el anclaje a la mediana de los compuestos del circuito, $\eta_\text{lo} = 0.985$, $\eta_\text{hi} = 1.04$ y $\Delta_\text{SC} = 60~\si{\second}$ un margen para neutralizaciones (\texttt{EXPECTED\_TIME\_LO\_FACTOR}, \texttt{EXPECTED\_TIME\_HI\_FACTOR}, \texttt{EXPECTED\_TIME\_SC\_MARGIN\_S}). Cuando el valor cae fuera de la cota se acota al extremo correspondiente y se marca \texttt{expected\_time\_clamped} en el payload, además de emitirse una entrada de log \texttt{strategy\_envelope\_clamp} con el contexto completo de la candidata. Si las capas anteriores funcionan, este paso debe activarse de manera infrecuente y los warnings sirven como instrumentación para detectar regresiones de modelo o de datos.
\end{enumerate}

\subsubsection{Validación cuantitativa}
\label{sssec:envoltura_validacion}

La incorporación progresiva de los cuatro mecanismos se midió sobre el mismo escenario de referencia (Sakhir 2023, 20 pilotos) mediante reejecuciones del diagnóstico de \S\ref{sssec:diagnostico_spread}. La tabla~\ref{tab:envoltura_validacion} resume la reducción de la dispersión $\Delta T = \max_i T_i - \min_i T_i$ entre pilotos para la estrategia ganadora de cada uno:

\begin{table}[h]
\centering
\caption{Dispersión de \texttt{expected\_time} entre pilotos al introducir progresivamente los clamps. La columna $\Delta T$ es la diferencia entre el máximo y el mínimo entre los 20 pilotos de Sakhir 2023.}
\label{tab:envoltura_validacion}
\begin{tabular}{lrr}
\toprule
Configuración & $\Delta T$ (s) & Reducción vs. baseline \\
\midrule
Baseline (sin clamps) & 8\,343 & ref. \\
\;+\;Corrección térmica acotada       & 3\,630 & $-56~\%$ \\
\;+\;Envoltura del Motor 1 y curvas    & 3\,029 & $-64~\%$ \\
\;+\;Clamp en \textit{rollouts} MC     &    895 & $-89~\%$ \\
\;+\;Envoltura del tiempo total        &    375 & $-95.5~\%$ \\
\bottomrule
\end{tabular}
\end{table}

La configuración final sitúa el rango de duraciones de carrera en $[1\text{:}32\text{:}18, 1\text{:}38\text{:}33]$, con mediana $1\text{:}35\text{:}28$ y ritmo medio $100.5~\si{\second\per vuelta}$, valores coherentes con la duración real del Gran Premio de Baréin de 2023 ($1\text{:}33\text{:}56$ para el ganador, \needcite{datos oficiales FIA 2023}). Los CSV intermedios de cada fase se conservan bajo \texttt{reports/pace\_spread\_*.csv} y permiten reproducir la trazabilidad completa del análisis.
