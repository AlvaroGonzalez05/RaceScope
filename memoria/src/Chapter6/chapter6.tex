...
En conjunto, cuatro de los cinco objetivos específicos se cumplen íntegramente y el quinto se cumple parcialmente en su componente más visible: la aplicación entregada cubre el caso de uso central pero deja abiertas las vistas en directo y de reproducción y el análisis de sensibilidad, que se trasladan a \ref{sec:trabajo_futuro}.
...
A medio plazo, el modelo de ritmo admite tres direcciones de expansión. Primero, incorporar más temporadas a medida que estén disponibles, con especial atención al solapamiento entre años para amortiguar la deriva temporal observada en la evaluación de 2025. Con esto se deberá tener mucho cuidado, ya que el \needcite{cambio de normativa técnica de F1 a partir de 2026 (FIA)} hace que se empiece prácticamente de cero en términos de datos históricos, y que la ventana de entrenamiento se reduzca drásticamente. En parte por esto se ha decidido no incluir datos posteriores a 2025, y abarcar únicamente la generación de monoplazas de Fórmula~1 de efecto suelo (\textit{ground effect}) vigente desde 2022 hasta 2025 \needcite{reglamento técnico FIA 2022}.

Segundo, explorar variantes ligeras de la arquitectura Transformer (\needcite{Linformer}, \needcite{Performer}) y de modelos especializados en series temporales (\needcite{N-BEATS}, \needcite{PatchTST}), así como la integración de modelos fundacionales tipo \needcite{TimesFM} como base para \textit{transfer learning}. Tercero, añadir una capa explícita de cuantificación de incertidumbre, mediante \needcite{\textit{dropout} bayesiano (Gal y Ghahramani)} o un \textit{ensemble} de cabezas, que ofrezca al usuario no solo la estrategia recomendada sino una banda de confianza sobre el tiempo esperado.
...
La segunda es el modelado explícito de las interacciones estratégicas entre pilotos. Desde la \needcite{teoría de juegos (Osborne y Rubinstein; Fudenberg y Tirole)}, cada equipo decide paradas con información parcial sobre rivales (estado de neumático, combustible, tráfico) y la utilidad depende de las decisiones ajenas. Para duelos uno a uno puede usarse un \needcite{juego bayesiano estático (Harsanyi)}; para líder y perseguidor, un enfoque tipo \needcite{Stackelberg} resulta más adecuado. A escala de campeonato, el juego se repite y las reputaciones de los equipos pueden integrarse como priors. En la práctica esto se traduce en modelos de tráfico que cuantifican la pérdida por seguir a un rival o, de forma escalable, en \needcite{aprendizaje por refuerzo multiagente (Busoniu \textit{et al.})} para obtener políticas de equilibrio en simulaciones completas.
